...
export const SignupForm = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Determine role from signup response, stored localStorage, or fall back to payload
      let roleStr = data?.role || data?.user?.role || localStorage.getItem('role') || '';

      // Edge case: some backends don't return role/user in signup response.
      // In that case we silently log in to get the session.
      if (!roleStr) {
        try {
          const loginData = await login(formData.email, formData.password);
          roleStr = loginData?.role || loginData?.user?.role || localStorage.getItem('role') || '';
        } catch {
          // Auto-login failed — redirect to login page so user can proceed manually
