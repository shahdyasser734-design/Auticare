...
async function run() {
  try {
    ...
    console.log('[2] Completing booking 14...');
    const completeRes = await request('PATCH', `/bookings/${bookingId}/status`, { status: 'Completed' }, docHeaders);
    console.log('Complete status response:', completeRes.statusCode, completeRes.data);

    console.log('[3] Fetching dashboard as Doctor...');
