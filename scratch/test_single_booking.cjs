...
async function run() {
  try {
    ...
    const bookingId = 14; // Confirmed booking from previous run
    ...
    console.log(`[2] Querying GET /api/bookings/${bookingId} as Doctor...`);
    const res = await request('GET', `/bookings/${bookingId}`, null, docHeaders);
    console.log('GET booking response (RAW):', JSON.stringify(res.data, null, 2));
