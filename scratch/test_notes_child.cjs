...
async function run() {
  try {
    ...
    console.log(`[2] Querying GET /api/notes/child/${childId} as Doctor...`);
    const res = await request('GET', `/notes/child/${childId}`, null, docHeaders);
    console.log('GET notes response (RAW):', res.statusCode, JSON.stringify(res.data, null, 2));
