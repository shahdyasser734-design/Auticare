...
export const AuthLayout = ({ children }: AuthLayoutProps) => {
  ...
  if (isAuthenticated && user?.role) {
    ...
    if (role === 'parent') return <Navigate to={ROUTES.PARENT_ADD_CHILD} replace />;
    if (role === 'doctor' || role === 'specialist') return <Navigate to={ROUTES.DOCTOR_HOME} replace />;
    if (role === 'therapist') return <Navigate to={ROUTES.THERAPIST_HOME} replace />;
  }
