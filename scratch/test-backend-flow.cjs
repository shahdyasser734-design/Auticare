...
async function run() {
  try {
    ...
    console.log('Full questions response:', JSON.stringify(qRes.data, null, 2));
    ...
    const lowAnswers = questionsList.map((q, i) => {
      return {
        ...
        Answer: 0,
        OptionId: `${q.questionId || (i + 1)}_no`
      };
    });
    console.log('Submitting low answers:', lowAnswers);
    // Transform to camelCase: { childId, answers: [ { questionId, answer, optionId } ] }
    const camelLowAnswers = lowAnswers.map(a => ({
    ...
    const camelHighAnswers = highAnswers.map(a => ({
      questionId: a.QuestionId,
      answer: a.Answer,
      optionId: a.OptionId
    }));
