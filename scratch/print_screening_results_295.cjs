...
async function run() {
  try {
    ...
    console.log(`[2] Querying GET /api/screening/results/${childId} as Doctor...`);
    const res = await request('GET', `/screening/results/${childId}`, null, docHeaders);
    console.log('GET screening results response (RAW):', res.statusCode, JSON.stringify(res.data, null, 2));
