...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signup = async (payload: Record<string, unknown>) => {
    ...
    try {
      ...
      // Build a user object from the response — some backends embed it in data.user,
      // others return the fields at the top level.
      ...
      if (userObj) {
        normalizeAndSetUser(userObj as any);
        // Also persist the role explicitly so navigation helpers can read it
        const rawRole = userObj.role || data.role || '';
        if (rawRole) localStorage.setItem('role', String(rawRole));
        localStorage.setItem('isAuthenticated', 'true');
        setIsAuthenticated(true);
      } else if (data.role) {
        // At minimum, save the role so the form can navigate correctly
